...
// Blockera webbåtkomst - detta skript ska bara köras via CLI, eller inkluderas
// av trigger_ai_processor.php när bakgrundsstarten misslyckats (konstanten kan
// bara sättas av serverkod, en direkt HTTP-request stoppas fortfarande).
if (php_sapi_name() !== 'cli' && !defined('STIMMA_JOB_PROCESSOR_INTERNAL')) {
...
VIKTIGT: Du MÅSTE generera EXAKT {$lessonCount} lektioner i lessons-arrayen. Räkna noga!";
    ...
    // Filimport (PPTX/SCORM): course_description är formaterad med markörer,
    // "SLIDE N: ..." respektive "SCO N: ...", följt av text och ev.
    // "BILDFIL:"/"VIDEOFIL:"-rader. Vi instruerar AI att behålla strukturen så
    // cron-koden kan mappa media till rätt lektion efteråt.
    $importMarker = detectImportMarker((string)$job['course_description']);
    if ($importMarker === 'SLIDE') {
    ...
    } elseif ($importMarker === 'SCO') {
        $scoCount = countImportBlocks((string)$job['course_description'], 'SCO');
        $structureSystemPrompt .= "\n\nKURSBESKRIVNINGEN ÄR EXTRAHERAD UR ETT SCORM-PAKET (SCO-MARKÖRER):\n"
            . "- Texten är råtext ur paketets sidor. Navigationsetiketter (\"Nästa\", \"Tillbaka\", \"Meny\") och upprepade rubriker kan förekomma — ignorera dem.\n"
            . "- Skriv om innehållet till sammanhängande lektionstext på svenska. Hitta inte på fakta som saknas i underlaget.\n";
        if ($scoCount === $lessonCount) {
            $structureSystemPrompt .= "- Behåll strukturen — varje SCO motsvarar exakt en lektion (i samma ordning).\n"
                . "- Använd SCO-titeln som lektionstitel om den är meningsfull. Titlar av formen 'Modul — Avsnitt' kommer från paketets innehållsträd; använd den sista delen som titel.\n";
        } else {
            $structureSystemPrompt .= "- Paketet innehåller {$scoCount} SCO(er) men kursen ska ha EXAKT {$lessonCount} lektioner — dela upp texten i logiska avsnitt i den ordning den står.\n";
        }
        $structureSystemPrompt .= "- Behåll referenserna 'BILDFIL: <namn>' och 'VIDEOFIL: <namn>' EXAKT i description-fältet för den lektion de hör till, så systemet kan mappa rätt media till rätt lektion.\n";
    }
...
box — bryt mellan boxarna, inte inom dem.';
    ...
    // SCORM-import: skicka bara den del av källtexten som hör till lektionen.
    // Underlaget kan vara tiotusentals tecken och hela klumpen i varje
    // lektionsprompt skulle både kosta orimligt mycket och dränka det som
    // faktiskt gäller just den lektionen.
    $importSlices = ($importMarker === 'SCO')
        ? buildImportSourceSlices((string)$job['course_description'], 'SCO', $totalLessons)
        : [];
    ...
    for ($i = 0; $i < $totalLessons; $i++) {
        ...
        $sourceText = $importSlices[$i] ?? (string)$job['course_description'];
        $sourceLabel = isset($importSlices[$i]) ? 'Källtext ur SCORM-paketet' : 'Kursbeskrivning';
...
Skriv ett komplett, informativt och engagerande lektionsinnehåll i HTML.";
    ...
    // Filimport (PPTX/SCORM): parse:a course_description efter "BILDFIL:"- och
    // "VIDEOFIL:"-rader per block och uppdatera respektive lektions media.
    // Blockordning matchar lessons.sort_order eftersom Phase 1 fick instruktion
    // att behålla 1:1-mappningen (vid uppdelning hamnar mediat på de första
    // lektionerna, vilket är rätt ordning men inte exakt block-för-block).
    $importMarker = detectImportMarker((string)$job['course_description']);
    if ($importMarker !== null) {
        $blocks = preg_split('/\n' . $importMarker . ' \d+:/', "\n" . $job['course_description']);
        // Första blocket är tomt pga splitten; resten är block i ordning.
        $blockImages = [];
        $blockVideos = [];
        foreach ($blocks as $idx => $block) {
            if ($idx === 0 || trim($block) === '') continue;
            if (preg_match('/BILDFIL:\s*([^\s\n]+)/', $block, $m)) {
                $blockImages[$idx - 1] = trim($m[1]);
            }
            if (preg_match('/VIDEOFIL:\s*([^\s\n]+)/', $block, $m)) {
                $blockVideos[$idx - 1] = trim($m[1]);
            }
        }
        if (!empty($blockImages) || !empty($blockVideos)) {
            ...
            foreach ($createdLessons as $i => $row) {
                if (isset($blockImages[$i])) {
                    execute(
                        "UPDATE " . DB_DATABASE . ".lessons SET image_url = ? WHERE id = ?",
                        [$blockImages[$i], $row['id']]
                    );
                }
                if (isset($blockVideos[$i])) {
                    // Videor ur paketet ligger i upload/videos/ → video_type 'local'
                    execute(
                        "UPDATE " . DB_DATABASE . ".lessons SET video_url = ?, video_type = 'local' WHERE id = ?",
                        [$blockVideos[$i], $row['id']]
                    );
                }
            }
            echo "  - Mappade " . count($blockImages) . " bilder och " . count($blockVideos)
               . " videor från {$importMarker}-import till lektioner.\n";
        }
    }
 ...
 */
/**
 * Vilken filimport kommer beskrivningen ifrån? 'SLIDE' = PowerPoint,
 * 'SCO' = SCORM, null = vanlig AI-generering.
 */
function detectImportMarker(string $description): ?string {
    if (strpos($description, 'SLIDE 1:') === 0) return 'SLIDE';
    if (strpos($description, 'SCO 1:') === 0)   return 'SCO';
    return null;
}

/**
 * Dela upp en importerad kursbeskrivning i en textbit per lektion.
 *
 * Är antalet block lika med antalet lektioner blir det 1:1. Annars (ett enda
 * SCO som delats upp i flera lektioner) delas texten sekventiellt vid
 * radbrytningar i ungefär lika stora delar — lektionerna följer källtextens
 * ordning, så bit N hör till lektion N.
 *
 * @return array Textbit per lektionsindex, eller tom array om underlaget saknas.
 */
function buildImportSourceSlices(string $description, string $marker, int $totalLessons): array {
    if ($totalLessons < 1 || $description === '') return [];

    $blocks = preg_split('/\n' . preg_quote($marker, '/') . ' \d+:/', "\n" . $description);
    $blocks = array_values(array_filter(array_slice($blocks, 1), fn($b) => trim($b) !== ''));
    if (empty($blocks)) return [];
    if (count($blocks) === $totalLessons) {
        return array_map('trim', $blocks);
    }

    $lines = preg_split('/\n/', trim(implode("\n", $blocks)));
    $totalChars = 0;
    foreach ($lines as $line) $totalChars += mb_strlen($line) + 1;
    if ($totalChars === 0) return [];

    $target = (int)ceil($totalChars / $totalLessons);

    // Enstaka rader kan vara längre än en hel lektionsandel (JS-drivna paket
    // levererar ofta ett helt avsnitt som en rad). Bryt dem vid meningsslut,
    // annars blir det färre bitar än lektioner.
    $pieces = [];
    foreach ($lines as $line) {
        if (mb_strlen($line) <= $target) { $pieces[] = $line; continue; }
        foreach (preg_split('/(?<=[.!?…])\s+/u', $line) as $sentence) {
            if ($sentence !== '') $pieces[] = $sentence;
        }
    }
    ...
    foreach ($pieces as $piece) {
        $current .= $piece . "\n";
        if (mb_strlen($current) >= $target && count($slices) < $totalLessons - 1) {
            $slices[] = trim($current);
            $current = '';
        }
    }
    if (trim($current) !== '') $slices[] = trim($current);

    // Räcker bitarna ändå inte till alla lektioner får de sista dela den sista
    // biten — bättre än att falla tillbaka på hela underlaget.
    while (count($slices) > 0 && count($slices) < $totalLessons) {
        $slices[] = $slices[count($slices) - 1];
    }

    return $slices;
}

/** Antal block ("SLIDE N:"/"SCO N:") i en importerad kursbeskrivning. */
function countImportBlocks(string $description, string $marker): int {
    return preg_match_all('/(^|\n)' . preg_quote($marker, '/') . ' \d+:/', $description);
}
