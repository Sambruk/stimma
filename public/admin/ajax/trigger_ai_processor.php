...
// Loggkatalog: upload/ under webbroten (public/upload). Före omläggningen av
// webbroten 2026-08-17 låg dokumentroten på /var/www/html och sökvägen
// '/var/www/html/upload' pekade på rätt katalog — efteråt pekar den på en
// tom, root-ägd katalog som www-data inte får skriva i. Då misslyckades
// shell-omdirigeringen nedan och bakgrundsprocessen startade aldrig.
$logDir = realpath(__DIR__ . '/../../upload');
if ($logDir === false || !is_writable($logDir)) {
    $logDir = sys_get_temp_dir();
}
$logFile = rtrim($logDir, '/') . '/ai_processor_' . date('Y-m-d_H-i-s') . '.log';
...
if ($jobStatus && $jobStatus['status'] === 'processing' || $logExists) {
    ...
    set_time_limit(600);
    ob_start();
    // Processorn blockerar webbåtkomst (php_sapi_name() !== 'cli'). Konstanten
    // signalerar att anropet kommer inifrån servern — en direkt HTTP-request
    // mot processorn kan inte sätta den och stoppas fortfarande med 403.
    define('STIMMA_JOB_PROCESSOR_INTERNAL', true);
